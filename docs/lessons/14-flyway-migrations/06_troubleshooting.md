# Lección 12 — Troubleshooting: Errores Comunes
...
Migration checksum mismatch for migration V1__Crear_tabla_tickets.sql
```
...
❌ **No hagas esto:**
```sql
-- V1__Crear_tabla_tickets.sql (ORIGINAL - ya ejecutado)
...
✅ **Haz esto:**
```sql
-- V1__Crear_tabla_tickets.sql (ORIGINAL - sin cambios)
CREATE TABLE tickets (id INT, title VARCHAR);

-- V2__Agregar_columna_descripcion.sql (NUEVA migración)
...
└── db/
    └── migration/
        └── V1__Crear_tabla.sql (sin subcarpeta mysql/supabase)
```
...
```
org.flywaydb.core.api.FlywayException: 
Unable to parse statement in migration file 'V1__Crear_tabla.sql'
```
...
-- V2__Crear_tabla_tickets.sql
CREATE TABLE tickets (id INT, user_id INT);

-- V3__Agregar_fk_usuarios.sql
ALTER TABLE tickets ADD CONSTRAINT fk_user
FOREIGN KEY (user_id) REFERENCES users(id);
```
...
```sql
-- V10__Eliminar_columna_vieja.sql
...
```sql
-- V10__Marcar_columna_como_deprecated.sql
